...
`src/vectorizer.rb` transforms any valid transaction payload hash into the exact 14-dim float array defined in DETECTION_RULES.md, including the `-1` sentinel for `last_transaction: null`.
...
Run with: `bundle exec ruby -Itest test/m02_vectorization_test.rb`

```ruby
# test/m02_vectorization_test.rb
